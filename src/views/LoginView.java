...
//                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    public static class AdminPanel extends JPanel {
        ...
        private void showAddMovieDialog() {
            ...
            JButton save=accentBtn("Lưu phim",Theme.ACCENT); 
            ...
            save.addActionListener(e->{
                ...
                Movie m=new Movie(
                		...
                		dirF.getText().trim(),
                		actF.getText().trim(),
                		new Category(catF.getText().isEmpty()?"Chung":catF.getText().trim(), 0),
                		cntF.getText().trim(),
        ...
        public void loadMovies(List<Movie> list) {
            ...
            for(Movie m:list) movieTableModel.addRow(new Object[]{
            		m.getId(),
            		m.getNameMovie(),
            		m.getDirector(),
            		m.getActor(),
            		m.getCategory()!=null ? m.getCategory().getCategory() : "-",
            				m.getCountry(),
            				m.getLink(),
            				m.isVip()?"✓":""}
            );
            totalMoviesLbl.setText(String.valueOf(list.size())); 
            updateStats();
        }

        public void loadUsers(List<Member> list) { 
        	this.users=list; 
        	refreshUserTable(); 
        }
        ...
        private void refreshUserTable() {
            ...
            for (Member u:users) userTableModel.addRow(
            		new Object[]{
            				u.getId(),
            				u.getEmail(),
            				u.getAccountStatus(),
        ...
        private void updateStats() {
            ...
            vipUsersLbl.setText(
            		String.valueOf(
            				users.stream().filter(u->"VIP".equalsIgnoreCase(u.getAccountStatus())).count()
            		)
            );
            lockedUsersLbl.setText(
            		String.valueOf(
            				users.stream().filter(u->"LOCKED".equalsIgnoreCase(u.getAccountStatus())).count()
            		)
            );
        }
